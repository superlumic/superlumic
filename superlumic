...
if [ -f "config/$USER.yml" ]; then
    setStatusMessage "Running the ansible playbook for $USER"
    ansible-playbook -i "localhost," config/$USER.yml
else
    if [ "travis" = "$USER" ]; then
        setStatusMessage "Running the ansible playbook for $USER but use roderik.yml as fallback"
        ansible-playbook -i "localhost," config/roderik.yml
    else
        triggerError "No playbook for $USER found"
    fi
fi
